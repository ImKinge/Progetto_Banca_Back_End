...
@Table(name = "card_transaction", schema = "banca")
@Entity
public class CardTransaction {
    ...
    @Column(name = "date_transaction", insertable = false, columnDefinition = "TIMESTAMP")
    ...
    @ManyToOne
    @JoinColumn(name = "orderer", referencedColumnName = "fiscal_code")
    private CustomerData customerDataOr;

    @ManyToOne
    @JoinColumn(name = "beneficiary",referencedColumnName = "fiscal_code")
    ...
    public CardTransaction(Integer id, Double amount, LocalDateTime dateTransaction, String descriptionTransaction, CustomerData customerDataOr, CustomerData customerDataBf) {
        this.id = id;
        this.amount = amount;
